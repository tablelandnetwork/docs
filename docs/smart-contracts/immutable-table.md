...
description: Permanently freeze a table's data by sending it to the zero address.
keywords:
  - read table
  - SQL query
---
...
If you want to "lock" access to the table so that _no one_ can ever own that table again, you can "burn" the token. To burn a token is to permanently and irreversibly remove that token from circulation, and this can be done by transferring the token to `0x00...0` (the zero address).
...
:::warning
The following walkthrough will send your table to the `0x0` address, so DO NOT transfer any `tableId` you wish to keep. Tableland has no way of retrieving a burned table; it will be immutable and forever owned by the `0x0` address.
:::
...
You can then import and set up the interface into your contract. One option is to set up an interface with the registry by importing and instantiating `ITablelandTables`, along with the [deployed contract's address](smart-contracts/contracts.md). Namely, you'd create a state variable like `ITablelandTables private _tableland` and set it equal to `ITablelandTables(address)`.
...
```solidity
TablelandDeployments.get().safeTransferFrom(DOCS_ADDR, 0x0000000000000000000000000000000000000000, 1);
```
...
7. Each of the accordions are methods of the Tableland registry smart contract. Scroll down to either [`safeTransferFrom`](https://goerli.etherscan.io/address/0xda8ea22d092307874f30a1f277d1388dca0ba97a#writeProxyContract#F8) or [`transferFrom`](https://goerli.etherscan.io/address/0xda8ea22d092307874f30a1f277d1388dca0ba97a#writeProxyContract#F13).
   ...
   3. The address it should be sent `to`, which is [`0x0000000000000000000000000000000000000000`](https://etherscan.io/address/0x0000000000000000000000000000000000000000)).
8. Proceed with the wallet flow by clicking _Confirm_—this will sign the transaction and send it to the blockchain. In other words, by clicking _Confirm_, you’ve authorized the table can be burned and will send it to the `0x0` address where _no one will ever be able to transfer or alter it_ thereafter. Only proceed if you wish to lose ownership forever!
