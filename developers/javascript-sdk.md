...
```typescript
const remove = await runQuery(`DELETE FROM ${id} WHERE id = 0;`);
```
...
```typescript
const { rows, columns } = await runQuery(`SELECT * FROM ${id};`);
```
